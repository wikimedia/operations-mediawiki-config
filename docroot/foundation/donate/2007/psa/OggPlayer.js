...
var wgOggPlayer = {
	...
	'players': ['gnash', 'flash', 'cortado', 'quicktime-mozilla', 'quicktime-activex', 'vlc-mozilla', 'vlc-activex', 'oggPlugin', 'videoElement'],
	...
	'init': function ( player, params ) {
		...
		switch ( player ) {
			case 'gnash':
				this.embedFlash( elt, params, true ); // gnash is pickier
				break;
			case 'flash':
				this.embedFlash( elt, params, false );
				break;
			case 'videoElement':
	...
	'detect': function( elt ) {
		...
		if(navigator.mimeTypes && navigator.mimeTypes.length > 0) {
			for ( var i = 0; i < navigator.mimeTypes.length; i++) {
				...
				var pluginDesc = navigator.mimeTypes[i].enabledPlugin ? navigator.mimeTypes[i].enabledPlugin.description : '';
				if ( !pluginDesc ) {
					pluginDesc = '';
				}
				...
				if ( type == 'application/x-shockwave-flash' ) {
					if (pluginDesc.indexOf( 'Gnash' ) > -1 ) {
						if (pluginDesc.match(/Gnash (0\.[0-7]|0\.8\.0)/)) {
							// Too-old version of gnash -- skip it!
						} else {
							// Video support is functional as of 0.8.1
							// ... but the nice FlowPlayer doesn't work.
							// Use a simpler workaround player.
							this.clientSupports['flash'] = false;
							this.clientSupports['gnash'] = true;
						}
						continue;
					}
					if (pluginDesc.match(/Flash [0-6]/)) {
						// flash 6 not supported
					} else {
	...
	'embedFlash': function ( elt, params, useGnash ) {
		var id = elt.id + "_obj";
		var swf = useGnash
			// God-damned hack
			? 'http://wikimediafoundation.org/donate/2007/psa/mongo-psa-gnash.swf'
			: 'http://wikimediafoundation.org/donate/2007/psa/FlowPlayerBlack.swf';
		//var vid = useFlv ? params.videoUrl.replace(/\.ogg/,'.flv') : params.videoUrl;
		var vid = params.videoUrl.replace(/\.ogg/,'.flv');
		vid = vid + '?' + Math.random(); // hack for possible squid buggage -- the main URL gets cut off oddly
		elt.innerHTML += 
			"<div><object id=" + this.hq( id ) + 
			" type='application/x-shockwave-flash' data='" + swf + "' " +
	...
	'embedCortado' : function ( elt, params ) {
		...
		var html =
		    ...
		    '      width=' + this.hq( params.width ) +
		    '      height=' + this.hq( playerHeight + statusHeight ) + 
