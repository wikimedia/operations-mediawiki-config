...
class UpdateWikiversions(cli.Application):
    ...
    def update_wikiversions(self):
        ...
        if os.path.exists(json_path):
            with open(json_file) as json_in:
                version_rows = json.read(json_in)
        else:
    ...
    def update_branch_pointer(self):
        branch_dirs = tasks.get_wikiversions_ondisk(self.config['stage_dir'])
        ...
        # FIX THIS ITS NOT EVEN PYTHON LOL YOU DOOF
        sorted(branch_dirs, key=lambda v: v[1]))
        current = branch_dirs.pop()[0]
        ...
        php_link_target = current[len(self.config['deploy_dir']) + 1:]
        utils.move_symlink(
            os.path.join(self.config['stage_dir'], 'php'), php_link_target)
