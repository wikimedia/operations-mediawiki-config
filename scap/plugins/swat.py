...
from scap.plugins.gerrit import gerrit_encoder, json_object_hook, person
...
#        help='Rebase the patch against this commit before merging.')
    ...
    def query(self, changeids):
        ...
        for changeid in changeids:
            ...
            cmdline = gerrit('query', ' --current-patch-set', '--format=JSON',
                '--submit-records', changeid)
            res = subprocess.check_output(cmdline)
            for line in res.splitlines():
                data = json.loads(line, object_hook=json_object_hook)
                if data.has_key('type') and data['type'] == 'stats':
                    continue
                import pprint
                self.dump_json(data)
