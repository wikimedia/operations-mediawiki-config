...
class Swat(cli.Application):
    ...
    '''
    @cli.argument('changeids', nargs='+', help='Change-id or gerrit patch url')
    ...
    @cli.argument('--rebase', action='store_true',
                  help='Rebase the change onto the tip of the target branch.')
    def pick(self, command):
        ...
        '''
        changeids = self.arguments.changeids
        for changeid in changeids:
    ...
    def query(self, args):
        ...
        query = ' '.join(self.arguments.term)
        changes = Changes()
