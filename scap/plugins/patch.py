...
class SecurityPatchManager(cli.Application):
    ...
    @cli.argument('-p', '--dir', nargs=1, required=False,
                  default='/srv/patches', metavar='PATH',
                  type=arg.is_dir,
                  help='Directory containing the patches to apply.')
    @cli.argument('-c', '--core', action='store_true',
                  help='Patch mediawiki/core')
    @cli.argument('-e', '--extensions', action='store_true',
                  help='Patch extensions')
    @cli.argument('--check-only', action='store_true',
                  help='Just check if patches apply cleanly.')
    @cli.argument('branch', nargs=1, type=arg.is_dir,
    ...
    def check_patch(self, patch):
        try:
            subprocess.check_output(['git', 'apply', '--check', '-3', patch])
    ...
    def apply_patch(self, patch, repo_dir='./'):
        with utils.cd(repo_dir):
            ...
            if not self.arguments.check_only:
                print('In %s, Applying patch: %s' % (repo_dir, patch))
                try:
                    subprocess.check_output(['git', 'am', '-3', patch])
                except subprocess.CalledProcessError as ex:
                    msg = 'Patch %s failed to apply: %s' % (patch, ex.output)
                    raise PatchError(msg, patch)
