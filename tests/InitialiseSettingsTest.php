	...
	public function testOnlyExistingWikis() {
		...
		foreach ($wgConf->settings as $config) {
			foreach ($config as $db => $entry) {
				...
				$this->assertTrue( in_array( $dbNormalized , $dblistNames ) || DBList::isInDblist( $dbNormalized, "all" ) || $db == "default", "$dbNormalized is referenced, but it isn't either a wiki or a dblist" );
			}
		}
	}
}
