	...
	private function setSiteInfoForWiki( $serverName, $docRoot ) {
		...
		if ( $secure ) {
			...
			if ( getenv( 'MW_LANG' ) ) {
				...
				$lang = getenv( 'MW_LANG' );
			} elseif ( preg_match( '/^(?:\/srv\/deployment\/mediawiki\/|\/usr\/local\/apache\/)(?:htdocs|common\/docroot)\/([a-z]+)\.org/', $docRoot, $matches ) ) {
			...
			} elseif ( preg_match( "/^(?:\/srv\/deployment\/mediawiki\/|\/usr\/local\/apache\/)(?:htdocs|common\/docroot)\/([a-z0-9\-_]*)$/", $docRoot, $matches ) ) {
				$site = "wikipedia";
				$lang = $matches[1];
			} else {
