...
global $wgWMFCluster, $wgWMFDatacenter, $wgWMFRealm;

$wgWMFCluster = trim( file_get_contents( '/etc/wikimedia-cluster' ) );
if ( $wgWMFCluster === 'labs' ) {
    $wgWMFRealm = 'labs';
    $wgWMFDatacenter = 'eqiad';
} else {
    $wgWMFRealm = 'production';
    $wgWMFDatacenter = $wgWMFCluster;
}
...
function getRealmSpecificFilename( $filename ) {
	...
	$new_filename = "{$base}-{$wgWMFRealm}-{$wgWMFDatacenter}{$ext}";
	if ( file_exists( $new_filename ) ) {
		return $new_filename;
	}

	# realm take precedence over datacenter.
	$new_filename = "{$base}-{$wgWMFRealm}{$ext}";
	if ( file_exists( $new_filename ) ) {
		return $new_filename;
	}

	$new_filename = "{$base}-{$wgWMFDatacenter}{$ext}";
	if ( file_exists( $new_filename ) ) {
		return $new_filename;
	}
