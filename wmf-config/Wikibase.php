...
if ( $wmgUseWikibaseRepo ) {
	...
	if ( $wgDBname === 'testwikidatawiki' ) {
		...
		$wgWBRepoSettings['clientDbList'] = array_diff(
			MWWikiversions::readDbListFile( getRealmSpecificFilename( "$IP/../dblist/wikidataclient.dblist" ) ),
			array( 'testwikidatawiki', 'testwiki', 'test2wiki' )
		);
		// Exclude closed wikis
		$wgWBRepoSettings['clientDbList'] = array_diff(
			$wgWBRepoSettings['clientDbList'],
			MWWikiversions::readDbListFile( getRealmSpecificFilename( "$IP/../dblist/closed.dblist" ) )
		);
	}
