...
if ( ini_get( 'hhvm.stats.enable_hot_profiler' ) ) {
	 ...
	 * - 'sampleprofiler': One-off sampling profile to stdout.
	...
	if ( isset( $xwd['sampleprofiler'] ) ) {
		...
		register_shutdown_function( function () {
			...
			// ```
			// [
			// "1526998006.300000": "main()==>{internal}==>run_init::/srv/mediawiki/php-master/load.php==>run_init::/srv/mediawiki/php-master/includes/WebStart.php==>ResourceLoader::respond==>ResourceLoader::getCombinedVersion==>array_map==>ResourceLoader::__invoke==>ResourceLoaderModule::getVersionHash==>ResourceLoaderStartUpModule::getDefinitionSummary==>ResourceLoaderStartUpModule::getAllModuleHashes==>ResourceLoader::getCombinedVersion@1==>array_map@1==>ResourceLoader::__invoke@1==>ResourceLoaderModule::getVersionHash@1==>ResourceLoaderModule::getModuleContent==>ResourceLoaderModule::buildContent==>ResourceLoaderFileModule::getStyles==>ResourceLoaderFileModule::readStyleFiles==>ResourceLoaderFileModule::readStyleFile==>ResourceLoaderFileModule::getRemotePath",
			// ]
			// ```

			// Prepare the data for our Wikimedia's flame graph pipeline
