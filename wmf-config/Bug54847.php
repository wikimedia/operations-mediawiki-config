...
$wgHooks['AbortChangePassword'][] = function ( $user, $password, $newpassword, &$errorMsg ) {
	...
	if ( !efUserIsAffected( $user, $loggedout, $isGlobal ) || ( isset( $user->Bug54847 ) && $user->Bug54847 === true ) ) {
		return true;
	}
	...
	$egBug54847 = array( 'name' => $user->getName(), 'wikis' => $resetWikis );

	return true;

};

$wgHooks['UserSaveSettings'][] = function ( $user ) {
	global $egBug54847;
	...
	if ( $egBug54847 !== false ) {
		...
		$dbw->update(
			...
			array( 'r_username' => $egBug54847['name'],
				'r_wiki' => $egBug54847['wikis']
			),
			__METHOD__
		);
	}
