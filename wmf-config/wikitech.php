...
// permablocked.
$wgHooks['BlockIpComplete'][] = function ( $block, $user, $prior ) use ( $PHAB_API_TOKEN ) {
	if ( $PHAB_API_TOKEN
		...
		try {
			...
			$phabClient = function ( $path, $query ) use ( $PHAB_API_TOKEN ) {
				$query['__conduit__'] = [ 'token' => $PHAB_API_TOKEN ];
				$post = [
